...
const groupOpportunitiesByStage = (opportunities) => {
    ...
    if (!opportunities || opportunities.length === 0) { 
        console.log('⚠️ GROUPING FUNCTION: No opportunities to group, returning empty data');
        return emptyData; 
    }

    const grouped = opportunities.reduce((acc, op) => {
        console.log(`📌 GROUPING FUNCTION: Processing opportunity "${op.contact_name}" with stage "${op.stage}"`);
        
        if (acc[op.stage]) {
            acc[op.stage].push(op);
            console.log(`✅ GROUPING FUNCTION: Added to stage "${op.stage}", now has ${acc[op.stage].length} opportunities`);
        } else {
            console.error(`❌ GROUPING FUNCTION: Unknown stage "${op.stage}" for opportunity "${op.contact_name}"`);
...
async function systematicDebug() {
    try {
        ...
        console.log(`📊 Found ${allOpps?.length || 0} total opportunities in database`);
        if (allOpps && allOpps.length > 0) {
            allOpps.forEach((opp, index) => {
                console.log(`  ${index + 1}. ID: ${opp.id}, Contact: "${opp.contact_name}", Stage: "${opp.stage}", Org: ${opp.organization_id}, Value: €${opp.value}`);
            });
        }
        ...
        if (orgs && orgs.length > 0) {
            for (const org of orgs) {
                ...
                console.log(`🏢 Organization "${org.name}" (${org.id}): ${orgOpps?.length || 0} opportunities`);
                
                if (orgOpps && orgOpps.length > 0) {
                    console.log('   📋 Opportunities details:');
                    orgOpps.forEach((opp, index) => {
                        console.log(`     ${index + 1}. Contact: "${opp.contact_name}", Stage: "${opp.stage}", Value: €${opp.value}`);
                    });
        ...
        profiles?.forEach((profile, index) => {
            console.log(`  ${index + 1}. User ID: ${profile.id}, Org ID: ${profile.organization_id}`);
        });
