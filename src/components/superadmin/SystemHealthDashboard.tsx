...
const SystemHealthDashboard: React.FC = () => {
  ...
  // Generate realistic timestamps in Rome timezone (UTC+2)
  const generateRomeTimestamp = (minutesAgo: number = 0): string => {
    const now = new Date();
    const romeTime = new Date(now.getTime() - (minutesAgo * 60 * 1000));
    // Format as ISO string but with Rome timezone offset
    ...
    const seconds = String(romeTime.getSeconds()).padStart(2, '0');
  ...
  const fetchSystemHealth = useCallback(async () => {
    try {
      ...
      const mockHealth: SystemHealth = {
        ...
        slowQueries: [
          { query: 'SELECT * FROM orders WHERE...', duration: 2340, timestamp: generateRomeTimestamp(4), table: 'orders' },
          { query: 'UPDATE users SET...', duration: 1890, timestamp: generateRomeTimestamp(6), table: 'users' }
        ],
        errors: [
          { endpoint: '/api/payments', error_message: 'Connection timeout', status_code: 500, created_at: generateRomeTimestamp(2), severity: 'high' }
        ],
        alerts: [
          { id: '1', message: 'High error rate detected on /api/orders', alert_type: 'warning', created_at: generateRomeTimestamp(4), resolved: false }
        ]
      };
