...
async function executeMigration() {
  ...
  try {
    ...
    if (!connectionConfig.password) {
      ...
      console.log('⚠️ No DB_PASSWORD found, attempting connection with service key...');
      // For Supabase, sometimes the service key can be used differently
      // We'll try the connection and see what happens
    }
    
    console.log(`✅ Connection target: ${connectionConfig.host}:${connectionConfig.port}`);
    ...
    try {
      ...
      if (connectionError.message.includes('password')) {
        console.log('⚠️ Password authentication failed, trying alternative approach...');
    ...
    columns.rows.forEach(col => {
      if (['id', 'organization_id', 'contact_id', 'policy_number'].includes(col.column_name)) {
...
executeMigration()
  ...
  .catch(error => {
    ...
    console.log('👉 Execute: supabase/migrations/20251018000000_insurance_policies_FIXED.sql');
    process.exit(1);
  });