...
ADD COLUMN IF NOT EXISTS vertical TEXT DEFAULT 'standard';
...
ALTER TABLE organizations
ADD CONSTRAINT valid_vertical CHECK (
    vertical IN (
        'standard',
        'insurance',
        'marketing',
...
-- Usage Stats
usage_count INTEGER DEFAULT 0, rating DECIMAL(3, 2),

-- Versioning
version INTEGER DEFAULT 1, changelog TEXT,
...
COMMENT ON
TABLE vertical_configurations IS 'Master configuration for each CRM vertical';

COMMENT ON
TABLE workflow_templates_global IS 'Global workflow templates cloned to new organizations';

COMMENT ON
TABLE email_templates_global IS 'Global email templates cloned to new organizations';

COMMENT ON
TABLE form_templates_global IS 'Global form templates cloned to new organizations';
