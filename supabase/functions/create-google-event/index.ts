...
serve(async (req) => {
  ...
  try {
    const { organization_id, contact_id, eventDetails, contact } = await req.json();
    
    if (!organization_id || !contact_id) {
      throw new Error("organization_id and contact_id are required at root level");
    }
    
    if (!eventDetails || !contact) {
      throw new Error("Dati mancanti: 'eventDetails' e 'contact' sono obbligatori.");
    }
