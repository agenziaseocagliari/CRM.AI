...
const SUPABASE_URL =
  process.env.VITE_SUPABASE_URL || 'https://qjtaqrlpronohgpfdxsi.supabase.co';
...
async function testProfileLookup(testUser) {
  ...
  try {
    ...
    if (!serviceKey) {
      colorLog(
        'yellow',
        '   ⚠️ No service role key - skipping direct verification'
      );
    } else {
      ...
      if (!profile) {
        colorLog(
          'red',
          `   ❌ FAILED: Profile not found for ${testUser.email}`
        );
        return false;
      }
      ...
      Object.entries(checks).forEach(([key, passed]) => {
        if (passed) {
          ...
          console.log(
            `      Expected: ${testUser[`expected${key.charAt(0).toUpperCase() + key.slice(1)}`]}`
          );
    ...
    colorLog(
      'yellow',
      '   To enable full testing, set SUPABASE_SERVICE_ROLE_KEY in .env'
    );
...
async function runAllTests() {
  ...
  if (passedCount === totalCount) {
    colorLog(
      'green',
      `🎉 ALL TESTS PASSED: ${passedCount}/${totalCount} (${passRate}%)`
    );
  } else {
    colorLog(
      'red',
      `⚠️ SOME TESTS FAILED: ${passedCount}/${totalCount} passed (${passRate}%)`
    );
  }
  colorLog('magenta', '='.repeat(80) + '\n');
