...
async function executeDiscovery() {
  try {
    ...
    if (tablesError) {
      ...
      const tableChecks = ['contacts', 'opportunities', 'deals', 'pipeline_stages', 'contact_notes', 'organizations']
    ...
    try {
      ...
      if (stagesError) {
        ...
        stages?.forEach(stage => {
          console.log(`  - ${stage.name} (ID: ${stage.id}) [Order: ${stage.display_order}] [Color: ${stage.color}]`)
        })
        
        // Find New Lead stage
        const newLeadStage = stages?.find(s => s.name.toLowerCase().includes('new') && s.name.toLowerCase().includes('lead'))
        if (newLeadStage) {
