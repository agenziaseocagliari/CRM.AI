...
CREATE POLICY "contact_notes_all_authenticated" ON public.contact_notes FOR ALL TO authenticated USING (true)
WITH
    CHECK (true);
...
CREATE POLICY "opportunities_all_authenticated" ON public.opportunities FOR ALL TO authenticated USING (true)
WITH
    CHECK (true);
...
INSERT INTO
    public.pipeline_stages (name, display_order, color)
VALUES ('New Lead', 1, '#3B82F6'),
    ('Contacted', 2, '#8B5CF6'),
    ('Proposal Sent', 3, '#F59E0B'),
    ('Won', 4, '#10B981'),
    ('Lost', 5, '#EF4444') ON CONFLICT (name) DO NOTHING;
...
CREATE POLICY "pipeline_stages_read_authenticated" ON public.pipeline_stages FOR
...
-- Count tables
SELECT 'contact_notes' as table_name, COUNT(*) as row_count, 'Table exists and accessible' as status
FROM public.contact_notes
UNION ALL
SELECT 'opportunities' as table_name, COUNT(*) as row_count, 'Table exists and accessible' as status
FROM public.opportunities
UNION ALL
SELECT 'pipeline_stages' as table_name, COUNT(*) as row_count, 'Table exists and accessible' as status
FROM public.pipeline_stages;

-- Show pipeline stage IDs (COPY THESE TO PHASE0_DISCOVERY_RESULTS.md)
SELECT 'NEW_LEAD_ID = ' || id as new_lead_info
FROM public.pipeline_stages
WHERE
    name = 'New Lead';
