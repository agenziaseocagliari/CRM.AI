...
CREATE INDEX IF NOT EXISTS idx_vertical_configurations_active ON vertical_configurations (is_active)
WHERE
    is_active = true;
...
COMMENT ON
TABLE vertical_configurations IS 'Master configuration table for CRM verticals. Defines UI layout, enabled modules, and default settings for each vertical type (Standard, Real Estate, etc.)';
