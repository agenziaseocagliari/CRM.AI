...
CREATE POLICY "contact_notes_org_access"
...
ALTER TABLE public.opportunities 
ADD COLUMN IF NOT EXISTS status text DEFAULT 'open' CHECK (status IN ('open', 'won', 'lost'));
...
SELECT 
    'NEW_LEAD_STAGE_ID = ' || id as new_lead_info
FROM public.pipeline_stages 
WHERE name ILIKE '%lead%' OR name ILIKE '%new%'
ORDER BY order_index
LIMIT 1;
