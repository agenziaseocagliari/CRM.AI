...
CREATE INDEX idx_insurance_policies_contact ON insurance_policies (contact_id);
...
CREATE POLICY "Users can view their organization's policies" ON insurance_policies FOR
...
CREATE POLICY "Users can create policies for their organization" ON insurance_policies FOR
INSERT
    TO public
WITH
    CHECK (
...
CREATE POLICY "Users can update their organization's policies" ON insurance_policies FOR
...
CREATE POLICY "Users can delete their organization's policies" ON insurance_policies FOR DELETE TO public USING (
    organization_id IN (
        SELECT organization_id
        FROM profiles
        WHERE
...
WHERE
    tc.table_name = 'insurance_policies'
