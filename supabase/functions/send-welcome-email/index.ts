...
const ACTION_TYPE = 'send_welcome_email';
...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      throw new Error("Mancano le variabili d'ambiente necessarie (SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY).");
    }
    ...
    if (!newContact || !newContact.email || !newContact.organization_id) {
        return new Response(JSON.stringify({ error: "Dati del contatto non validi." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    // --- Integrazione Sistema a Crediti (con client admin) ---
     const { data: creditData, error: creditError } = await supabaseAdmin.functions.invoke('consume-credits', {
        body: { organization_id: newContact.organization_id, action_type: ACTION_TYPE },
    });
    ...
    if (creditData && !creditData.success) {
      console.warn(`[${ACTION_TYPE}] Crediti insufficienti per l'organizzazione ${newContact.organization_id}. Email di benvenuto non inviata.`);
      return new Response(JSON.stringify({ success: true, message: "Crediti insufficienti, email non inviata." }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200,
      });
    }
    console.log(`[${ACTION_TYPE}] Crediti verificati per ${newContact.organization_id}. Rimanenti: ${creditData.remaining_credits}`);
    // --- Fine Integrazione ---
    ...
    if (!settings || !settings.brevo_api_key) {
        ...
        return new Response(JSON.stringify({ message: "Nessuna chiave API Brevo configurata." }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!geminiApiKey || !brevoSenderEmail || !brevoSenderName) {
        throw new Error("Le variabili d'ambiente GEMINI_API_KEY, BREVO_SENDER_EMAIL e BREVO_SENDER_NAME sono necessarie.");
    }
