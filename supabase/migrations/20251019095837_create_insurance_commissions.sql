...
CREATE INDEX IF NOT EXISTS idx_insurance_commissions_organization_id 
    ON insurance_commissions(organization_id);

CREATE INDEX IF NOT EXISTS idx_insurance_commissions_policy_id 
    ON insurance_commissions(policy_id) WHERE policy_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_insurance_commissions_contact_id 
    ON insurance_commissions(contact_id);

CREATE INDEX IF NOT EXISTS idx_insurance_commissions_status 
    ON insurance_commissions(status);

CREATE INDEX IF NOT EXISTS idx_insurance_commissions_calculation_date 
    ON insurance_commissions(calculation_date);

CREATE INDEX IF NOT EXISTS idx_insurance_commissions_commission_type 
    ON insurance_commissions(commission_type);

-- Create composite index for common queries
CREATE INDEX IF NOT EXISTS idx_insurance_commissions_org_status_date 
    ON insurance_commissions(organization_id, status, calculation_date DESC);
...
CREATE POLICY "insurance_commissions_select_org" ON insurance_commissions
    FOR SELECT USING (
        organization_id IN (
...
CREATE POLICY "insurance_commissions_insert_org" ON insurance_commissions
...
CREATE POLICY "insurance_commissions_update_org" ON insurance_commissions
    FOR UPDATE USING (
        organization_id IN (
            SELECT organization_id FROM profiles WHERE id = auth.uid()
        )
...
CREATE POLICY "insurance_commissions_delete_org" ON insurance_commissions
    FOR DELETE USING (
        organization_id IN (
            SELECT organization_id FROM profiles WHERE id = auth.uid()
        )
...
COMMENT ON TABLE insurance_commissions IS 'Insurance commission tracking and management';
